...
class IteratedRandomisedLinearDeltaModel(BaseModel, nn.Module):
    def __init__(self, best_response:BaseBestResponse, loss:BaseLoss, x_dim:int, num_comps:int=3, **kwargs):
        ...
        self._comp_index = 0
        self.mix_logits = torch.zeros(1) # Mixing distribution
    ...
    def forward_loss(self, X: Tensor) -> Tensor:
        weights = self.mean[self._comp_index,:]
        biases = self.bias[self._comp_index]

        X = X.unsqueeze(1)
        out = torch.einsum("boi,oi->bo", X, weights) + biases
        out = out.squeeze()
        return out 
    ...
    def fit(self, train_dset:BaseDataset, opt, lr, batch_size=128, epochs=100, verbose=False):
        ...
        for i in range(self.num_comps):
            ...
            if verbose:
                print(f"Comp: {i+1} training complete")
